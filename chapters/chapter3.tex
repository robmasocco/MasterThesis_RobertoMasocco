...
\begin{figure}
    ...
    \begin{tikzpicture}[->,
                        ...
                        node distance=6cm,
                        semithick]
        \node[state] (SETUP)                        {SETUP};
        \node[state] (RUNNING)   [right of=SETUP]   {RUNNING};
        \node[state] (TERMINATE) [right of=RUNNING] {TERMINATE};
        \path[]
            (SETUP)   edge                      node [above] {start mission}     (RUNNING)
            (RUNNING) edge [bend left=50]       node [above] {mission completed} (TERMINATE)
                      edge [bend left=25]       node [above] {takeoff fail}      (TERMINATE)
                      edge [bend right=25]      node [below] {land fail}         (TERMINATE)
                      edge [bend right=50]      node [below] {abort mission}     (TERMINATE);
    \end{tikzpicture}
    \caption{Diagramma di stato della FSM di alto livello.}
    \label{fig:highfsm}
\end{figure}
\clearpage
...
\begin{figure}
    ...
    \begin{tikzpicture}[->,
        ...
        \node[state, align=center] (NAV)  [right of=FT]        {NAVIGATING};
        \node[state, align=center] (EXPL) [right of=NAV]       {EXPLORING};
        \node[state, align=center] (PL)   [below of=NAV]       {PRECISION\\LANDING};
        \node[state, align=center] (LND)  [below right of=PL]  {LANDED};
        \node[state, align=center] (SP)   [below left of=PL]   {SEARCH\\PAD};
        \node[state, align=center] (TK)   [below of=EXPL]      {TAKEOFF};
        \path[]
            (FT)   edge [bend left=50]       node [above]     {start exploration}     (EXPL)
            (EXPL) edge [bend right=20]      node [above]     {sequence from gcs}     (NAV)
            (NAV)  edge [bend right=20]      node [left]      {pad reached}           (PL)
            (PL)   edge [bend right=20]      node [right=0.1] {change pad}            (NAV)
                   edge [bend right=20]      node [left=0.1]  {pad lost}              (SP)
                   edge [bend left=20]       node [right=0.1] {landing ok}            (LND)
            (LND)  edge [bend right=20]      node [right]     {new pad}               (TK)
            (SP)   edge [bend right=20]      node [below=0.6] {pad reached}           (PL)
                   edge [bend left=45]       node [left]      {change pad}            (NAV)
            (TK)   edge [bend right=25]      node [right=0.1] {takeoff ok}            (NAV);
    \end{tikzpicture}
    \caption{Diagramma di stato della meta-FSM contenuta nello stato \emph{RUNNING}.}
    \label{fig:metafsm}
\end{figure}
\clearpage
